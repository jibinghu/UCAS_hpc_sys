...
// Function to perform parallel Gauss-Seidel iteration
void parallelGaussSeidel(int n, const std::vector<double>& A, const std::vector<double>& b, std::vector<double>& x, int max_iters, double tol) {
    int rank, size;
    ...
    for (int k = 0; k < max_iters; ++k) {
        ...
        // Gauss-Seidel iteration for local part
        ...
        // Gather all local x parts to update global x
        MPI_Allgather(local_x.data(), local_n, MPI_DOUBLE, x.data(), local_n, MPI_DOUBLE, MPI_COMM_WORLD);

        // Check convergence
        double local_diff = 0.0;
        for (int i = 0; i < local_n; ++i) {
            local_diff += (local_x[i] - x_old[rank * local_n + i]) * (local_x[i] - x_old[rank * local_n + i]);
        }
...
int main(int argc, char* argv[]) {
    ...
    parallelGaussSeidel(n, A, b, x, max_iters, tol);
